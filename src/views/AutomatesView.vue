      ...
      <div v-for="automate in automates" :key="automate.id" 
        ...
        <div class="flex justify-between items-center">
          ...
          <button 
            ...
            @click="editAutomate(automate.id)"
          >
            Éditer
          </button>
        </div>
...
// Utiliser notre composable de chargement
...
const editAutomate = (id) => {
  ...
  // Affiche l'écran de chargement personnalisé pour l'édition
  const automate = automates.value.find(a => a.id === id);
  
  showLoading({
    title: `ÉDITION DE ${automate.name.toUpperCase()}`,
    messages: [
...
const createNewAutomate = () => {
  console.log('Création d\'un nouvel automate');
  // Affiche l'écran de chargement personnalisé pour la création
  showLoading({
    title: 'CRÉATION D\'AUTOMATE',
    messages: [
