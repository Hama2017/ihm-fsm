...
const dependenciesText = computed(() => {
  ...
  // Si plus de 2 dépendances, montrer "N clauses"
  if (props.edge.automataDependencies.length > 2) {
    return `${props.edge.automataDependencies.length} clauses`;
  }
  
  // Sinon, montrer les noms des automates
  const labels = props.edge.automataDependencies.map(dependencyId => {
    return getAutomataName(dependencyId);
  });
...
// Largeur approximative du texte des dépendances pour le rectangle de fond
const dependenciesLabelWidth = computed(() => {
  return Math.max(dependenciesText.value.length * 5.5, 60);
});
