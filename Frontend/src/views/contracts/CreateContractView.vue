...
>
  ...
  <div class="h-full">
    <FunctionList
      ...
      @edit-transition="openEditTransitionModal"
      @remove-transition="openRemoveTransitionModal"
      @reverse-transition="openInvertTransitionModal"
      @open-add-modal="openAddTransitionModal"
      @open-add-conditions="openAddConditionModal"
      @open-condition-info="openConditionInfoModal"
      @remove-condition="openRemoveConditionModal"
      @open-add-dependencies="openAddAutomataDependencyModal"
      @remove-dependency="removeAutomataDependency"
    />
  </div>
</template>
...
>
  <div class="space-y-6">
    ...
    <div class="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-800">
      ...
      <!-- ✅ NOUVEAU : Afficher le nom actuel -->
      <div class="text-center mt-2">
        <span class="text-xs text-gray-500 dark:text-gray-400">Nom actuel :</span>
        <code class="text-xs font-mono text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 px-2 py-1 rounded ml-1">
          {{ invertingTransition.label }}
        </code>
      </div>
    </div>
    ...
    <div class="border border-green-200 dark:border-green-800 rounded-lg p-4 bg-green-50 dark:bg-green-900/20">
      ...
      <!-- ✅ NOUVEAU : Afficher le nouveau nom généré -->
      <div class="text-center mt-2">
        <span class="text-xs text-green-600 dark:text-green-400">Nouveau nom :</span>
        <code class="text-xs font-mono text-green-700 dark:text-green-300 bg-white dark:bg-gray-700 px-2 py-1 rounded ml-1">
          {{ generateTransitionName(invertingTransition.target, invertingTransition.source) }}
        </code>
      </div>
    </div>
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800">
      <p class="text-xs text-blue-700 dark:text-blue-300">
        <strong>Note :</strong> Le nom de la fonction sera automatiquement régénéré selon le nouveau sens de la transition.
      </p>
    </div>
  </div>
</Modal>
...
<!-- Modifier le Modal d'ajout de transition pour supprimer le champ fonction -->
<Modal
...
>
  <div class="space-y-4">
    ...
    <!-- ✅ SUPPRIMÉ : Section Fonction/Déclencheur -->
    <!-- 
    <div>
      <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
        Déclencheur
    ...
    <!-- ✅ AJOUTÉ : Prévisualisation du nom généré -->
    <div v-if="newTransition.source && newTransition.target" class="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800">
      <label class="block text-xs font-medium text-blue-700 dark:text-blue-300 mb-1">
        Nom de fonction généré :
      </label>
      <code class="text-sm font-mono text-blue-800 dark:text-blue-200">
        {{ generateTransitionName(newTransition.source, newTransition.target) }}
      </code>
    </div>
...
<Modal
  ...
  confirm-text="Créer la transition"
  ...
  <div class="space-y-4">
    ...
    <div class="text-sm space-y-3">
      <div class="flex items-center">
        ...
        <span class="px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-md">
          {{ connectionModalData.sourceName }}
        </span>
      </div>
      <div class="flex items-center">
        <span class="font-medium w-24 text-gray-700 dark:text-gray-300">Vers:</span>
        <span class="px-2 py-1 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 rounded-md">
          {{ connectionModalData.targetName }}
        </span>
      </div>
    </div>
    
    <!-- Affichage du nom de fonction généré -->
    <div v-if="connectionModalData.source && connectionModalData.target" class="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg border border-purple-200 dark:border-purple-800">
      <label class="block text-xs font-medium text-purple-700 dark:text-purple-300 mb-1">
        Nom de fonction généré :
      </label>
      <code class="text-sm font-mono text-purple-800 dark:text-purple-200 bg-white dark:bg-gray-800 px-2 py-1 rounded">
        {{ generateConnectionTransitionName() }}
      </code>
      <p class="text-xs text-purple-600 dark:text-purple-400 mt-1">
        Ce nom sera utilisé comme fonction Solidity
      </p>
    </div>
    ...
    <div class="space-y-2">
      <div class="flex justify-between items-center">
        ...
        <span class="text-xs text-gray-500 dark:text-gray-400">
          {{ connectionSelectedConditions.length }} sélectionnée(s)
        </span>
      </div>
      ...
      <div class="border border-gray-200 dark:border-gray-700 rounded-lg max-h-60 overflow-y-auto">
        <div v-for="(packageC, index) in packetCondition" :key="packageC.id" class="border-b last:border-b-0 border-gray-200 dark:border-gray-700">
          <div 
            ...
            @click="toggleConnectionPackage(index)"
          >
            {{ packageC.label }}
            <div>
              <LucideChevronDown v-if="connectionExpandedPackages[index]" class="w-4 h-4 text-gray-600 dark:text-gray-400" />
              <LucideChevronRight v-else class="w-4 h-4 text-gray-600 dark:text-gray-400" />
            </div>
          </div>
          <div v-show="connectionExpandedPackages[index]" class="p-2 bg-white dark:bg-gray-800">
            <div v-for="condition in packageC.functions" :key="condition.id" class="mb-1 last:mb-0">
              <label class="flex items-center px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                <input 
                  type="checkbox" 
                  :value="condition.id" 
                  v-model="connectionSelectedConditions"
      ...
      <div v-if="connectionSelectedConditions.length > 0" class="mt-2 flex flex-wrap gap-1">
        <div 
          v-for="conditionId in connectionSelectedConditions" 
          :key="conditionId"
        ...
        >
          <span class="px-2 py-0.5 text-indigo-700 dark:text-indigo-300">
            {{ getConnectionConditionLabel(conditionId) }}
          </span>
          <button 
            @click="removeConnectionCondition(conditionId)"
    ...
    <p v-if="connectionModalError" class="text-sm text-red-600 dark:text-red-400 mt-2">
    ...
    >
      <div class="space-y-4">
        ...
        <div>
          ...
          <!-- ✅ NOUVEAU : Prévisualisation du nom généré -->
    <div v-if="editingTransition.source && editingTransition.target" class="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg border border-purple-200 dark:border-purple-800">
      <label class="block text-xs font-medium text-purple-700 dark:text-purple-300 mb-1">
        Nouveau nom de fonction :
      </label>
      <code class="text-sm font-mono text-purple-800 dark:text-purple-200 bg-white dark:bg-gray-800 px-2 py-1 rounded">
        {{ generateTransitionName(editingTransition.source, editingTransition.target) }}
      </code>
      <p class="text-xs text-purple-600 dark:text-purple-400 mt-1">
        Le nom sera automatiquement mis à jour
      </p>
    </div>
...
import {
  ...
  LucidePlayCircle, LucidePlay,LucideInfo, LucideChevronDown, LucideChevronRight 
...
// 4️⃣ Gestion des transitions - Union complète
const {
  normalizeStateName,
  generateTransitionName,
  // Propriétés de gestion des menus contextuels
  edgeContextMenu,
  ...
  // États d'édition et de transition
  newTransition,        // ← AJOUTEZ cette ligne
  editingTransition,
  invertingTransition,
  removeTransitionId,
  
  // Données de mise à jour des edges
  edgeUpdateData,
  edgeUpdateSourceName,
  edgeUpdateTargetName,
  
  // Gestion des conditions et automates
  selectedCondition,
  selectedConditions,
  selectedAutomates,
  ...
  // Erreurs des modals
  modalError,           // ← AJOUTEZ cette ligne
  editTransitionError,
  conditionModalError,
  dependencyModalError,
  
  // Méthodes de style et mise à jour
  updateEdgeStyles,
  
  // Méthodes de connexion et gestion des edges
  handleConnectNodes,
  onEdgeClick,
  handleEdgeUpdate,
  confirmEdgeUpdate,
  
  // Méthodes CRUD des transitions
  addTransition,
  editTransition,
  removeTransition,
  invertTransition,
  ...
  openAddTransitionModal,  // ← Cette ligne doit être présente
  ...
  // Méthodes pour gérer les conditions
...
function onNodeConnect(params) {
  ...
  // ✅ CORRECTION : Utiliser openConnectionModal au lieu de transitionModalRef
  openConnectionModal(
    result.source,
    result.target,
    result.sourceName,
...
function onTransitionConfirmOnConnect({ source, target, conditions }) {
  const { success, message } = addTransition({
    source,
    target,
    // ✅ SUPPRIMÉ : label: transition,
...
const confirmDeployment = async () => {
  try {
    ...
    // Rediriger directement vers la vue de déploiement
    const contractId = route.params.id
    router.push({
      name: 'deployment',
      params: { 
    ...
    // En cas d'erreur, supprimer le flow
    ...
    if (deploymentFlowIndex !== -1) {
      ...
      console.log('❌ Flow supprimé suite à l\'erreur');
    }
...
const cancelDeployment = () => {
  // ✅ MODIFIÉ: Supprimer le flow ET nettoyer la sauvegarde
  ...
  if (deploymentIndex !== -1) {
    ...
    // Sauvegarder sans le flow
    saveContract(true); // true = nettoyer le flow
  }
