<template>
      ...
      <div v-if="filteredContracts.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
            ...
            :class="[
              ...
              getStatusBarColor(contract.status)
            ]"
          ></div>
          ...
          <div class="pl-2">
            ...
            <div class="flex flex-col space-y-3 mb-5">
              ...
              <!-- Badge createdBy si disponible -->
              <div v-if="contract.createdBy" class="flex items-center text-sm">
                <LucideUser class="h-4 w-4 text-gray-500 dark:text-gray-400 mr-2 flex-shrink-0" />
  ...
  <!-- Monitoring (si completed - en vert) -->
  <button
    v-if="contract.status === 'completed'"
    @click.stop="executeContract(contract.id)"
    class="p-2 text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-full transition-colors"
    title="Surveiller le contrat"
  >
  ...
  <!-- Détails (si déployé OU completed) -->
  <button 
    v-if="contract.status === 'deployed' || contract.status === 'completed'"
  ...
  >
    ...
    >
      <div v-if="selectedContract" class="space-y-6">
        ...
        <div class="grid grid-cols-2 gap-6">
          ...
          <div v-if="selectedContract.createdBy">
            <span class="font-medium text-gray-900 dark:text-white">{{ t('contracts.createdBy') }}:</span>
            <span class="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300">
        ...
        <!-- Liste des automates (filtrée pour exclure flow-deploiement) -->
        <div v-if="getDisplayedAutomates(selectedContract).length > 0">
          <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Automates:</h4>
          <div class="space-y-3">
            <div 
              v-for="automate in getDisplayedAutomates(selectedContract)" 
              :key="automate.id"
    ...
    <!-- Écran de chargement pour les opérations -->
    <UiLoading 
...
const getStatusBarColor = (status) => {
  switch (status) {
    ...
    case 'draft':
    default:
      return 'bg-yellow-500';  // Jaune pour brouillon
  }
};
...
// Debug: Vérifier l'état d'auth en temps réel
...
// Log les infos d'auth pour debug
console.log('Auth debug info:', authDebugInfo.value);

// États - CORRECTION: Utiliser directement contracts du store
const contracts = computed(() => automatonContractStore.contracts);
const searchQuery = ref('');
const statusFilter = ref('all');
const fileInputRef = ref(null);
const isLoading = computed(() => automatonContractStore.loading); // Loading du store
const isLocalLoading = ref(false); // Loading local pour les opérations
...
const loadContracts = async () => {
  ...
  console.log('authStore.user keys:', authStore.user ? Object.keys(authStore.user) : 'no user');
  console.log('========================');

  // Vérifier que l'utilisateur est connecté
  if (!authStore.isAuthenticated) {
    console.warn('Utilisateur non authentifié');
    toast.error(t('errors.auth.not_authenticated'));
    return;
  }

  // Essayer de récupérer l'ID utilisateur selon différentes propriétés possibles
  ...
  if (!userID) {
    console.warn('ID utilisateur manquant. User object:', authStore.user);
    ...
    try {
      ...
      if (fetchResult.success) {
        const newuserID = authStore.user?.id || 
                         authStore.user?.userID || 
                         authStore.user?.user_id || 
                         authStore.user?.email ||
                         authStore.user?.username;
        ...
        if (!newuserID) {
          console.error('Aucun identifiant trouvé après fetchUser');
          toast.error('Impossible de récupérer l\'identifiant utilisateur');
          return;
        }
      } else {
        console.error('Échec de fetchUser:', fetchResult.errorCode);
  ...
  try {
    // Récupérer l'ID final après les tentatives de récupération
    const userID = authStore.user?.email;
    let result ;
    console.log('Chargement des contrats pour l\'utilisateur:', userID);

    if (authStore.user.role==='admin') {
      // Si l'utilisateur est admin, charger tous les contrats
      console.log('Utilisateur admin, chargement de tous les contrats');
      result = await automatonContractStore.fetchContracts();
    } else {
      // Sinon, charger les contrats de l'utilisateur spécifique
      console.log('Chargement des contrats pour l\'utilisateur:', userID);
      result =  await automatonContractStore.fetchContractsByUser(userID);
    }
    
    if (!result.success) {
      console.error('Échec du chargement des contrats:', result.errorCode);
      toast.error(t('errors.contract.load_failed'));
    } else {
      console.log('Contrats chargés avec succès:', result.data?.length || 0, 'contrats');
    }
  } catch (error) {
...
// Helper pour obtenir le nombre d'automates (excluant flow-deploiement)
const getAutomateCount = (contract) => {
  if (!contract.automates) return 0;
  
  // Si deployed, exclure le flow de déploiement du comptage
  if (contract.status === 'deployed'|| contract.status === 'completed' ) {
    return contract.automates.filter(automate => 
      automate.id !== 'flow-deploiement'
    ).length;
  }
  
  return contract.automates.length;
};

// Helper pour obtenir les automates à afficher (excluant flow-deploiement)
const getDisplayedAutomates = (contract) => {
  if (!contract.automates) return [];
...
const confirmDeleteContract = async () => {
  ...
  try {
    //  CORRECTION : Fermer le modal AVANT d'activer le loading
    ...
    // Désactiver le chargement local et nettoyer
...
const filteredContracts = computed(() => {
  ...
  return contracts.value.filter(contract => {
    ...
    const matchesStatus = statusFilter.value === 'all' ||
                         ...
                         (statusFilter.value === 'completed' && contract.status === 'completed'); 
...
const processContractImport = async (importedData) => {
  try {
    ...
    if (result.success) {
      ...
      // Recharger les contrats après l'import
      await loadContracts();
    } else {
...
onMounted(async () => {
  try {
    ...
    // Debug: Vérifier l'état d'auth au montage
    ...
    if (!authStore.user && authStore.isAuthenticated) {
      ...
      try {
        ...
        console.error('Impossible de récupérer les infos utilisateur:', error);
      }
    }
