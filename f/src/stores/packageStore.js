...
export const usePackageStore = defineStore('packages', {
  ...
  actions: {
    /**
     * Charge tous les packages depuis le serveur ou le localStorage.
    ...
    async fetchPackages() {
      ...
      try {
        ...
        // Extraire les packages selon différentes structures possibles
        if (packages && packages.data !== undefined) {
          console.log('Found response.data structure');
          packages = packages.data;
        }
        
        console.log('Packages after extraction:', packages);
        ...
        if (Array.isArray(packages)) {
          ...
          // Si packages n'est pas un tableau mais un objet unique, le traiter
          if (packages && typeof packages === 'object' && !Array.isArray(packages)) {
            console.log('Packages is a single object, converting to array');
            try {
          ...
          // Si on arrive ici, c'est que le format est vraiment invalide
        ...
        const errorCode = extractErrorCode(error, 'not_found');
        this.error = errorCode;
        
        return {
          success: false,
          errorCode: `errors.package.${errorCode}`
        };
      } finally {
        this.loading = false;
      }
    ...
    async createPackage(packageData) {
      ...
      try {
        ...
        const errorCode = extractErrorCode(error, 'already_exists');
        this.error = errorCode;
    ...
    async updatePackage(packageId, packageData) {
      ...
      try {
        ...
        const errorCode = extractErrorCode(error, 'update_failed');
        this.error = errorCode;
    ...
    async deletePackage(packageId) {
      ...
      try {
        ...
        const errorCode = extractErrorCode(error, 'deletion_failed');
        this.error = errorCode;
    ...
    exportPackage(packageId) {
      ...
      try {
        ...
        const errorCode = extractErrorCode(error, 'export_failed');
        this.error = errorCode;
    ...
    async importPackage(file) {
      ...
      try {
        ...
        const errorCode = extractErrorCode(error, 'import_failed');
        this.error = errorCode;
    ...
    async createFunction(packageId, functionData) {
      ...
      try {
        ...
        const existingFunction = pkg.functions.find(f => f.id === functionData.id);
        if (existingFunction) {
        ...
        const errorCode = extractErrorCode(error, 'function_creation_failed');
        this.error = errorCode;
    ...
    async updateFunction(packageId, functionId, functionData) {
      ...
      try {
        ...
        const errorCode = extractErrorCode(error, 'package.function_update_failed', 'package');
        this.error = errorCode;
    ...
    async deleteFunction(packageId, functionId) {
      ...
      try {
        ...
        const errorCode = extractErrorCode(error, 'package.function_deletion_failed', 'package');
        this.error = errorCode;
