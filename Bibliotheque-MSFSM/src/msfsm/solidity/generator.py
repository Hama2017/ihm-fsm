...
class GeneratorSolidity(Generator):
    ...
    def _set_structs(self, automaton_name: str) -> None:
        ...
        """
        used_packages = set(self.specification.used_packages.get(automaton_name, []))
    ...
    def _set_conditions_by_trigger_name(
            ...
            """
            # Variable pour tracker si on a trouvé une transition valide
            self.result[automaton_name] += "bool transitionExecuted = false;"
            
            # Pour la première transition
            if transitions[0].conditions:
                conditions = " && ".join([c + "()" for c in transitions[0].conditions])
                self.result[automaton_name] += f"if ({conditions} && currentState == State.{transitions[0].source.upper()}) {{"
            else:
                self.result[automaton_name] += f"if (currentState == State.{transitions[0].source.upper()}) {{"
            
            self.result[automaton_name] += f"currentState = State.{transitions[0].destination.upper()};"
            self.result[automaton_name] += "transitionExecuted = true;"
            self.result[automaton_name] += "}"

            for t in transitions[1:]:
                if t.conditions:
                    conditions = " && ".join([c + "()" for c in t.conditions])
                    self.result[automaton_name] += f"else if ({conditions} && currentState == State.{t.source.upper()}) {{"
                else:
                    self.result[automaton_name] += f"else if (currentState == State.{t.source.upper()}) {{"
                
                self.result[automaton_name] += f"currentState = State.{t.destination.upper()};"
                self.result[automaton_name] += "transitionExecuted = true;"
                self.result[automaton_name] += "}"
            
            self.result[automaton_name] += 'require(transitionExecuted, "No valid transition conditions met");'
    ...
    def _set_conditional_functions_by_trigger_name(
        ...
        for c in conditions:
            ...
            if condition_type == "package": 
                self.result[automaton_name] += (
                    self.packages[package_name].data.functions[function_name].code
                )
            elif condition_type == "automata":
                self.result[
                    automaton_name
                ] += f"function {c}() public view returns (bool) {{"
                self.result[
                    automaton_name
                ] += f'(bool success, bytes memory data) = address({self.deployed_smart_contract_info[package_name]["address"]}).staticcall(abi.encodeWithSignature("is_completed()"));'
                self.result[automaton_name] += 'require(success, "staticcall to is_completed failed");'
                self.result[automaton_name] += "return abi.decode(data, (bool));"
                self.result[automaton_name] += "}"
            else:
    ...
    def save_to_file(self, path: str = None):
        ...
        for automaton_name in self.result.keys():
            filename = f"{path}/{self.specification.data.name}/{automaton_name}.sol"
            os.makedirs(os.path.dirname(filename), exist_ok=True)
            with open(filename, "w") as f:
                f.write(self.result[automaton_name])