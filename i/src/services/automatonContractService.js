      ...
      if (Array.isArray(automate.transitions)) {
        automate.transitions.forEach(transition => {
          if (transition.source && transition.target) {
            ...
            // Ajouter les conditions par défaut seulement si aucune condition spécifique n'existe
            if (transitionObj.conditions.length === 0) {
              defaultConditions.forEach(defaultCondition => {
                transitionObj.conditions.push(defaultCondition);
                console.log(`Condition par défaut ajoutée pour ${sourceLabel} -> ${targetLabel}:`, defaultCondition);
                
                // Ajouter le package de la condition par défaut
                if (defaultCondition.startsWith('package__')) {
                  const parts = defaultCondition.split('__');
                  if (parts.length >= 2) {
                    packagesUsed.add(parts[1]);
                  }
                }
