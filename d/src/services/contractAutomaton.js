...
const ContractAutomatonService = {
   ...
   * @returns {Promise}
   */
  updateContractAutomaton(contractName, contractData) {
    return apiClient.put(`/automaton-contracts/${contractName}`, contractData);
  },
  ...
  deployContract(specificationModel) {
    return withMinDelay(
      apiClient.post('/smart-contracts/deploy', specificationModel),
      MIN_OPERATION_DELAY * 5 // Délai plus long pour le déploiement
    );
  },
