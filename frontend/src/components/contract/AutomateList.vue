...
const filteredAutomates = computed(() => {
  return sortedAutomates.value
    .filter(automate => automate.id !== 'flow-deploiement') // Exclure le flow
    .filter(automate => {
      if (!searchQuery.value.trim()) return true; // Pas de recherche = tout afficher
      return automate.name.toLowerCase().includes(searchQuery.value.toLowerCase().trim());
    });
});
