...
export default function useState({
  ...
  /**
   * Vérifie si un état est l'état COMPLETED (état protégé)
   * @param {String} stateId - ID de l'état à vérifier
   * @returns {Boolean} True si c'est l'état COMPLETED
   */
  const isCompletedState = (stateId) => {
    const node = nodes.value.find(n => n.id === stateId);
    return node && node.data.label === 'COMPLETED';
  };

  /**
   * Génère un nom unique pour un état en fonction du type
   * @param {String} type - Type d'état ('initial', 'standard', 'final')
   * @returns {String} Nom unique généré
   */
  const generateUniqueStateName = (type) => {
    let baseName;
    switch (type) {
      case 'initial':
        baseName = 'État Initial';
        break;
      case 'final':
        baseName = 'État Final';
        break;
    ...
    // Pour les états standard, on commence directement avec "État1"
    if (type === 'standard') {
      candidateName = `${baseName}${counter}`;
    }
    
    // Trouver un nom unique
    while (stateNameExists(candidateName)) {
      counter++;
      if (type === 'standard') {
        candidateName = `${baseName}${counter}`;
      } else {
  ...
  /**
   * Génère un ID unique pour un nouvel état
  ...
  const openEditStateModal = (id) => {
    try {
      // Vérifier si c'est l'état COMPLETED
      if (isCompletedState(id)) {
        toast.error('Impossible de modifier l\'état COMPLETED');
        return;
      }
  ...
  const openRemoveStateModal = (id) => {
    try {
      // Vérifier si c'est l'état COMPLETED
      if (isCompletedState(id)) {
        toast.error('Impossible de supprimer l\'état COMPLETED');
        return;
      }
  ...
  const editState = ({ id, libelle }) => {
    // Vérifier si c'est l'état COMPLETED
    if (isCompletedState(id)) {
      return { success: false, message: 'Impossible de modifier l\'état COMPLETED' };
    }
  ...
  const removeState = (id) => {
    // Vérifier si c'est l'état COMPLETED
    if (isCompletedState(id)) {
      return { success: false, message: 'Impossible de supprimer l\'état COMPLETED' };
    }
  ...
  const confirmAddState = () => {
    try {
      ...
      const newId = generateUniqueStateId();
  ...
  /**
   * Ajoute un état initial avec validation d'unicité
   * @returns {Object} Résultat de l'opération
   */
  const addInitialState = () => {
    try {
      const id = generateUniqueStateId();
      const libelle = generateUniqueStateName('initial');
      ...
      const result = addState(newState);
      if (result.success) {
        toast.success('État initial ajouté avec succès');
      }
      return result;
    } catch (error) {
      console.error('Erreur lors de l\'ajout de l\'état initial:', error);
      toast.error('Erreur lors de l\'ajout de l\'état initial');
      return { success: false, message: 'Erreur lors de l\'ajout de l\'état initial' };
    }
  };

  /**
   * Ajoute un état standard avec validation d'unicité
   * @returns {Object} Résultat de l'opération
   */
  const addStandardState = () => {
    try {
      const id = generateUniqueStateId();
      const libelle = generateUniqueStateName('standard');
      ...
      const result = addState(newState);
      if (result.success) {
        toast.success('État standard ajouté avec succès');
      }
      return result;
    } catch (error) {
      console.error('Erreur lors de l\'ajout de l\'état standard:', error);
      toast.error('Erreur lors de l\'ajout de l\'état standard');
      return { success: false, message: 'Erreur lors de l\'ajout de l\'état standard' };
    }
  };

  /**
   * Ajoute un état final avec validation d'unicité
   * @returns {Object} Résultat de l'opération
   */
  const addFinalState = () => {
    try {
      const id = generateUniqueStateId();
      const libelle = generateUniqueStateName('final');
      ...
      const result = addState(newState);
      if (result.success) {
        toast.success('État final ajouté avec succès');
      }
      return result;
    } catch (error) {
      console.error('Erreur lors de l\'ajout de l\'état final:', error);
      toast.error('Erreur lors de l\'ajout de l\'état final');
      return { success: false, message: 'Erreur lors de l\'ajout de l\'état final' };
    }
  };
  ...
  return {
    ...
    generateUniqueStateName,
    generateUniqueStateId,
    isCompletedState,
